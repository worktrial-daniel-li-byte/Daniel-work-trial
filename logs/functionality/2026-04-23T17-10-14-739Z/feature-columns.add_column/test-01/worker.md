# Feature columns.add_column — attempt 01

## Task to worker

Write a new @playwright/test spec at tests/board/columns.add_column.spec.mjs that verifies the
feature check below.
...
I wrote `tests/board/columns.add_column.spec.mjs` with 9 `expect()` assertions covering the full mutate→verify→revert→verify cycle: it asserts the `'Create status'` button is visible, that clicking it opens a `role=dialog` with the correct heading, a visible `Name` textbox, and a disabled `Submit` button, then cancels the dialog and confirms it is hidden and all three original column headings (`To Do`, `In Progress`, `Done`) remain visible.
