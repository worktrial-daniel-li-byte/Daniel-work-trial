# Feature toolbar.view_settings — attempt 01

## Task to worker

Write a new @playwright/test spec at tests/board/toolbar.view_settings.spec.mjs that verifies the
feature check below.
...
I wrote `tests/board/toolbar.view_settings.spec.mjs` with a single test that follows the open→assert→revert→assert sequence: it clicks **View settings**, then asserts the `role=complementary` Sidebar is visible with the level-2 heading, **Resize sidebar** button, **Close** button, the populated **Show fields** group (including the always-enabled **Summary** checkbox), and the **Never** combobox; it then clicks **Close** and asserts the sidebar has `count(0)` and the toolbar button is no longer `aria-pressed=true`.
