# Feature columns.more_actions — mutation attempt 01
## Task to saboteur
Remove or disable the single feature described below so the spec at
tests/board/columns.more_actions.spec.mjs will FAIL. The harness will revert your edits after
...
Spec file (read-only — do NOT edit it): tests/board/columns.more_actions.spec.mjs

Keep the app compiling. Break only this one feature.
## Saboteur summary
The per-column More actions menu — including its trigger button (`aria-label="More actions for column {title}"`) and all menu items ("Add agent BETA", "Rename status", "Move column right", "Delete status") — is **not implemented in `src/`**. The source only has a `<span className="sr-only">More actions for column {group.title}</span>` placeholder (not a `<button>`), and the `columnMenuFor` / `onOpenColumnMenu` props are explicitly voided in `BoardView`. None of the expected menu-item strings appear anywhere in the codebase.

The feature lives entirely in the live Jira tenant (`fleet-team-y0ak1u2s.atlassian.net`) where the browser evidence was gathered. Editing `src/` cannot make this spec fail (if the harness runs against live Jira, edits to local files have no effect; if it runs against `localhost`, the test was already failing before any sabotage). **I am making no edits.**
