# Feature card.open_work_item — mutation attempt 01
## Task to saboteur
...
## Saboteur summary
Removed the `href` attribute from the card `<a>` tag in `src/App.tsx` (line 2713). Without `href`, the element loses its implicit `role="link"`, so Playwright's `getByRole('link', { name: 'AUT-1 …' })` will no longer find it and the spec fails at assertion 2.
