# Feature card.open_work_item — mutation attempt 02
## Task to saboteur
...
## Saboteur summary
Edited `src/App.tsx` at line 2734: removed the `href` attribute from the card `<a>` element, so the anchor no longer has `href="/browse/AUT-1"`. This breaks the `a[href="/browse/AUT-1"]` CSS-attribute locator (assertions 2 & 3), removes the implicit link role so `getByRole('link', …)` won't find it (assertion 5), and makes the `toHaveAttribute('href', …)` check fail (assertion 3).
