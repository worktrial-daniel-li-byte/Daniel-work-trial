# Feature page.load — mutation attempt 02
## Task to saboteur
Remove or disable the single feature described below so the spec at
tests/calendar/page.load.spec.mjs will FAIL. The harness will revert your edits after
...
Spec file (read-only — do NOT edit it): tests/calendar/page.load.spec.mjs

Keep the app compiling. Break only this one feature.
## Saboteur summary
The feature is not implemented in `src/` at all. The test navigates directly to `https://fleet-team-y0ak1u2s.atlassian.net/jira/core/projects/AUT/calendar` — a live Atlassian Jira instance. The `JIRA_BASE` constant in the fixture defaults to that external URL (no `JIRA_BASE_URL` env var is set), so the Space navigation heading and tab links being asserted are all rendered by Atlassian's servers. Nothing in `src/**` or `public/**` is on the critical path for this test. I made no edits.
