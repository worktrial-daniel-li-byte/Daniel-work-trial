...
test.describe('toolbar.view_settings — View settings opens a right-side panel', () => {
  test('open panel shows heading, resize handle and close button; closing removes the panel', async ({ page }) => {
    ...
    // Show fields list has rendered. `exact: true` prevents matching
    // "Sub-task summary" which also contains the word "summary".
    await expect(
      sidebar.getByRole('checkbox', { name: 'Summary', exact: true })
    ).toBeVisible()

    // 'Hide done work items after:' section is present. The control is a
    // React Select whose hidden dummy <input> always has value="", so we
    // verify the label and the displayed option text instead of toHaveValue.
    await expect(
      sidebar.getByText('Hide done work items after:', { exact: true })
    ).toBeVisible()
    // The currently selected option 'Never' is rendered in the visible
    // single-value span next to the combobox, not in the input's value.
    await expect(sidebar.getByText('Never', { exact: true })).toBeVisible()
